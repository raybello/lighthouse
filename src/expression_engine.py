"""
Expression Engine for evaluating {{}} syntax expressions.
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                if isinstance(value, dict):
                    ...
                    setattr(self, key, [DictWrapper(item) if isinstance(item, dict) else item for item in value])
                else:
...
class ExpressionEngine:
    """
    Engine for parsing and evaluating expressions with {{}} syntax.
    ...
    def __init__(self, context: Optional[Dict[str, Any]] = None):
        ...
        Args:
            context: Dictionary mapping node IDs/names to their output data
        """
        self.context = context or {}
        
    def set_context(self, context: Dict[str, Any]) -> None:
        """
        Update the execution context with node outputs.
    ...
    def evaluate_expression(self, expression: str) -> Any:
        """
        Evaluate a single expression using the current context.
        ...
        try:
            ...
            temp_context = {}
            for node_name, node_data in self.context.items():
    ...
    def resolve(self, value: Any) -> Any:
        ...
        if single_expr_match:
            ...
            try:
                ...
                console.print(f"[red]Expression error: {e}[/red]")
                return value
        ...
        for expr in expressions:
            try:
                ...
                console.print(f"[red]Expression error in '{expr}': {e}[/red]")
    ...
    def resolve_dict(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                result[key] = [self.resolve(item) if not isinstance(item, dict) 
                              else self.resolve_dict(item) for item in value]
            else:
