...
class NodeBase(ABC):
        ...
        with dpg.node(
            ...
            with dpg.node_attribute(
                ...
                with dpg.group(horizontal=True):
                    ...
                    # Edit button - opens configuration inspector
                    # Only shown if node has configurable fields
                    dpg.add_button(
                        label="Edit",
            ...
            with dpg.node_attribute(
                ...
                with dpg.group(horizontal=True):
                    ...
                    dpg.add_text(
                        default_value=self.status, color=(101, 122, 231), tag=f"{self.id}_exec_status" 
                    )
                # Status text showing node ID (last 8 characters)
                dpg.add_text(
                    default_value=f"{self.id[-8:]}", tag=f"{self.id}_state", color=(86, 145, 193)
                )
    ...
    def setup_node_inspector(self) -> None:
        ...
        with dpg.window(
            ...
            dpg.add_input_text(default_value=self.name, tag=f"{self.id}_rename_text", label="Enter Name")
            dpg.add_button(label="Save", tag=f"{self.id}_rename_save_btn", callback=lambda: self.close_rename_popup())
...
class LighthouseApp:
    ...
    def __init__(
        self, title: str = "Lighthouse", width: int = 1400, height: int = 900
    ) -> None:
        """
    ...
    def _exec_graph(self, node_id):
        ...
        for nid in execution_order:
            ...
            if self.nodes[nid].status == "PENDING":
                ...
                self._execute_step(nid)
            elif self.nodes[nid].status == "COMPLETED" and started == False and nid == node_id:
                started = True
                self._execute_step(nid)
            else: 
                pass
